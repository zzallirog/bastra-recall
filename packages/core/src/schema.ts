...
/**
 * A declaration, not a stored result.  The resolver name is closed so vault
 * content cannot turn load_memory into a command runner.  Results are derived
 * on load and never written back into the memory.
 */
export const DerivedClaimSchema = z.object({
  ...
  /** Stable reference to the independent case; Bastra stores but never follows it. */
...
export const FrontmatterSchema = z.object({
  ...
  /** #467 follow-up — formulas, never cached values. */
